  ...
  const handleAuth = async () => {
    ...
    try {
      if (isRegistering) {
        ...
        // onboarding_completed: false → triggers OnboardingScreen after login
        await setDoc(doc(db, 'usuarios', user.uid), {
          ...
          notif_enabled:        true,
          dark_mode:            true,
          onboarding_completed: false,  // ← KEY: shows onboarding after register
          created_at:           new Date().toISOString(),
        });
      } else {
        await signInWithEmailAndPassword(auth, email.trim(), password);
        // Existing users: App.js reads onboarding_completed from Firestore
        // If the field doesn't exist (old account) it defaults to true → goes straight to app
      }
    } catch (error) {
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.logoSection, { opacity: logoOpacity, transform: [{ scale: logoScale }] }]}>
          ...
          <Text style={styles.appTagline}>
            {isRegistering ? 'Crea tu identidad.' : 'Bienvenido de vuelta.'}
          </Text>
        </Animated.View>
